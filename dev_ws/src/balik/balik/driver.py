...
class _Config:
    ...
    WAITCOEFF_TRANSLATION       = 4
    WAITCOEFF_ROTATION          = 27.5
        ...
        while True:
            ...
            if(_HMI.IsValid(currentinput)):
                ...
                if (currentinput == _Config.CONSOLE_TEST):
                    _Robot.Test()
                if (currentinput == _Config.CONSOLE_INIT):
                    _Robot.Init()
                else:
...
class DriverPublisher(Node):
    def __init__(self):
        super().__init__('driverpub')
        self.publisher1_ = self.create_publisher(Float32, 'balik_batteryvoltage', 10)
        self.publisher2_ = self.create_publisher(Float32, 'balik_xpos', 10)
        self.publisher3_ = self.create_publisher(Float32, 'balik_ypos', 10)
    ...
    def timer_callback(self):
        ...
        msg2 = Float32()
        msg3 = Float32()
        msg1.data = float(_Data.BATTERY)
        msg2.data = float(_Data.XPOS)
        msg3.data = float(_Data.YPOS)
...
def main(args=None):
    ...
    driverpub = DriverPublisher()
    driversub = DriverSubscriber()

    executor = rclpy.executors.MultiThreadedExecutor()
    executor.add_node(driverpub)
    executor.add_node(driversub)
    executor_thread = Thread(target=executor.spin, daemon=True)
    executor_thread.start()
    rate = driverpub.create_rate(2)
